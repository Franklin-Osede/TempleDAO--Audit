...
contract SpiceAuctionTestBase is TempleGoldCommon {
  ...
  event AuctionConfigSet(
    uint256 epoch,
    ISpiceAuction.SpiceAuctionConfig config
  );
  ...
  function setUp() public {
    ...
    factory = new SpiceAuctionFactory(
      rescuer,
      executor,
      daoExecutor,
      mike,
  ...
  function _startAuction(
    ...
    if (_sendAuctionTokens) {
      address auctionToken = config.isTempleGoldAuctionToken
        ? address(templeGold)
        : spice.spiceToken();
  ...
  function _getAuctionToken(
    bool _isTempleGoldAuctionToken,
    address _spiceToken
...
contract SpiceAuctionViewTest is SpiceAuctionTestBase {
  function test_getAuctionTokenForCurrentEpoch() public {
    ...
    address auctionToken = _config.isTempleGoldAuctionToken
      ? address(templeGold)
      : spice.spiceToken();
...
contract SpiceAuctionTest is SpiceAuctionTestBase {
  function test_setDaoExecutor() public {
    vm.startPrank(daoExecutor);
    vm.expectRevert(
      abi.encodeWithSelector(CommonEventsAndErrors.InvalidAddress.selector)
    );
  ...
  function test_setSpiceAuctionConfig() public {
    ...
    ISpiceAuction.SpiceAuctionConfig memory _config = spice.getAuctionConfig(
      epochId + 1
    );
    ...
    vm.startPrank(daoExecutor);
    vm.expectRevert(
      abi.encodeWithSelector(ISpiceAuction.InvalidConfigOperation.selector)
    );
    spice.setAuctionConfig(config);
  }
  ...
  function test_removeAuctionConfig() public {
    ...
    ISpiceAuction.SpiceAuctionConfig memory _emptyConfig = spice
      .getAuctionConfig(2);
  ...
  function test_recoverAuctionTokenForZeroBidAuction() public {
    ...
    uint256 auctionTokenBalance = IERC20(auctionToken).balanceOf(
      address(spice)
    );
    ...
    assertEq(
      IERC20(auctionToken).balanceOf(address(spice)),
      auctionTokenBalance - auctionTokenAmount
    );
    assertEq(
      IERC20(auctionToken).balanceOf(alice),
      aliceBalance + auctionTokenAmount
    );
    ...
    spice.claim(1);
    assertEq(
      IERC20(auctionToken).balanceOf(bob),
      bobBalance + epochOneTotalAuctionTokenAmount
    );
    assertEq(spice.epochsWithoutBidsRecovered(2), true);

    vm.startPrank(daoExecutor);
    vm.expectRevert(
      abi.encodeWithSelector(IAuctionBase.AlreadyRecovered.selector)
    );
  ...
  function test_recoverToken_spice() public {
    ...
    assertEq(
      recoverAmount + daoDaiBalanceBefore,
      IERC20(daiToken).balanceOf(daoExecutor)
    );
    ...
    vm.expectRevert(
      abi.encodeWithSelector(ISpiceAuction.RemoveAuctionConfig.selector)
    );
    ...
    assertEq(
      aliceBidTokenBalance + recoverAmount,
      spiceTokenErc20.balanceOf(alice)
    );
    assertEq(
      spiceBidTokenBalance - recoverAmount,
      spiceTokenErc20.balanceOf(address(spice))
    );
    ...
    assertEq(
      spiceTempleGoldBalance - recoverAmount,
      templeGold.balanceOf(address(spice))
    );
    assertEq(
      aliceTempleGoldBalance + recoverAmount,
  ...
  function test_startSpiceAuction() public {
    ...
    vm.expectRevert(
      abi.encodeWithSelector(IAuctionBase.CannotStartAuction.selector)
    );
    spice.startAuction();
    ...
    vm.expectRevert(
      abi.encodeWithSelector(ISpiceAuction.NotEnoughAuctionTokens.selector)
    );
    spice.startAuction();

    IERC20 auctionToken = IERC20(
      _getAuctionToken(_config.isTempleGoldAuctionToken, daiToken)
    );
    ...
    assertEq(
      epochInfo.startTime,
      uint128(block.timestamp + _config.startCooldown)
    );
    assertEq(
      epochInfo.endTime,
      uint128(epochInfo.startTime + _config.duration)
    );
    ...
    dealAdditional(
      IERC20(_getAuctionToken(_config.isTempleGoldAuctionToken, daiToken)),
      address(spice),
      50 ether
    );
  ...
  function test_bidSpiceAuction() public {
    ...
    vm.startPrank(alice);
    vm.expectRevert(
      abi.encodeWithSelector(IAuctionBase.CannotDeposit.selector)
    );
    spice.bid(10 ether);
    ...
    assertEq(IERC20(daiToken).balanceOf(alice), 100 ether - aliceBidAmount);
    assertEq(
      IERC20(daiToken).balanceOf(_config.recipient),
      aliceBidAmount + bobBidAmount
    );
    epochInfo = spice.getEpochInfo(epoch);
    ...
    spice.bid(50 ether);
    assertEq(
      spice.getAuctionBidAmount(1),
      50 ether + aliceBidAmount + bobBidAmount
    );
    assertEq(spice.getAuctionTokenAmount(1), 100 ether);
    assertEq(IERC20(daiToken).balanceOf(alice), 100 ether - aliceBidAmount);
    assertEq(
      IERC20(daiToken).balanceOf(treasury),
      aliceBidAmount + bobBidAmount + 50 ether
    );
    epochInfo = spice.getEpochInfo(epoch);
    assertEq(
  ...
  function test_claimSpiceAuction() public {
    ...
    vm.expectRevert(
      abi.encodeWithSelector(IAuctionBase.CannotClaim.selector, 1)
    );
    spice.claim(1);
    vm.warp(epochInfo.startTime);
    vm.expectRevert(
    ...
    epoch = spice.currentEpoch();
    vm.expectRevert(
      abi.encodeWithSelector(IAuctionBase.CannotClaim.selector, epoch)
    );
    spice.claim(epoch);
    epochInfo = spice.getEpochInfo(epoch);
    vm.warp(epochInfo.endTime + _config.waitPeriod);
    ...
    vm.expectRevert(
      abi.encodeWithSelector(IAuctionBase.AlreadyClaimed.selector)
    );
    spice.claim(epoch);
  }
  ...
  function test_claim_tgld_is_bid_token() public {
    ...
    uint256 totalTgldBid = (spice.getEpochInfo(spice.currentEpoch()))
      .totalBidTokenAmount;
    ...
    vm.expectRevert(
      abi.encodeWithSelector(IAuctionBase.CannotClaim.selector, currentEpoch)
    );
  ...
  function test_burn_and_notify_arbitrum() public {
    ...
    vm.expectRevert(
      abi.encodeWithSelector(ISpiceAuction.EtherNotNeeded.selector)
    );
  ...
  function test_mint_max_supply_update_circulating_supply() public {
    ...
    assertEq(
      ...
      templeGold.MAX_CIRCULATING_SUPPLY()
    );
  ...
  function test_vulnerabilityLowParticipationAuction() public {
    // Configurar subasta
    ISpiceAuction.SpiceAuctionConfig memory _config = _startAuction(true, true);
    uint256 epoch = spice.currentEpoch();
    IAuctionBase.EpochInfo memory epochInfo = spice.getEpochInfo(epoch);

    // Avanzar al inicio de la subasta
    vm.warp(epochInfo.startTime);

    // Alice participa con una oferta pequeña
    deal(daiToken, alice, 1 ether);
    vm.startPrank(alice);
    IERC20(daiToken).approve(address(spice), type(uint).max);
    uint256 aliceBidAmount = 0.01 ether;
    spice.bid(aliceBidAmount);

    // Avanzar casi al final de la subasta
    vm.warp(epochInfo.endTime - 1 hours);

    // Bob participa con una oferta aún más pequeña
    deal(daiToken, bob, 1 ether);
    vm.startPrank(bob);
    IERC20(daiToken).approve(address(spice), type(uint).max);
    uint256 bobBidAmount = 0.001 ether;
    spice.bid(bobBidAmount);

    // Verificar proporciones antes del final
    uint256 aliceShare = spice.getClaimableForEpoch(alice, epoch);
    uint256 bobShare = spice.getClaimableForEpoch(bob, epoch);

    // Avanzar al final de la subasta
    vm.warp(epochInfo.endTime + _config.waitPeriod);

    // Reclamar y verificar cantidades
    vm.startPrank(alice);
    spice.claim(epoch);
    vm.startPrank(bob);
    spice.claim(epoch);

    // Mostrar la desproporción
    // bobShare debería ser aproximadamente 9% del total a pesar de que apostó solo 9% de lo que apostó Alice
    assertApproxEqRel(
      bobShare,
      (spice.getAuctionTokenAmount(epoch) * 9) / 100,
      0.01e18
    );
  }
}
